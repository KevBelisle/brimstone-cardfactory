...
var CardView = Backbone.View.extend({
  ...
  render: function () {
    ...
    if (Object.keys(requiredText).length > 0) {
      this.$el.append(
        "<div id='requiredText'><h3>Required Text Components</h3><ul></ul></div>",
      );
    ...
    if (bodyTextOptions.length > 0) {
      this.$el.append(
        "<div id='bodyText'><h3>Body Text Components</h3><ul></ul></div>",
      );
    ...
    if (Object.keys(toggleableText).length > 0) {
      this.$el.append(
        "<div id='toggleableText'><h3>Toggleable Text Components</h3><ul></ul></div>",
      );
    ...
    if (Object.keys(toggleableIcons).length > 0) {
      this.$el.append(
        "<div id='toggleableIcons'><h3>Toggleable Icons</h3><ul></ul></div>",
      );
    ...
    if (Object.keys(userImages).length > 0) {
      this.$el.append(
        "<div id='userImages'><h3>User Image</h3><ul></ul></div>",
      );
    ...
    if (backgroundOptions.length > 0) {
      this.$el.append(
        "<div id='backgroundOptions'><h3>Background Options</h3><fieldset><ul></ul></fieldset></div>",
      );
  ...
  renderRequiredText: function (text) {
    this.$("#requiredText ul").append(
      new RequiredTextView({
        tagName: "li",
        model: text,
      }).el,
    );
  ...
  renderToggleableText: function (text) {
    this.$("#toggleableText ul").append(
      new ToggleableTextView({
        tagName: "li",
        model: text,
      }).el,
    );
  ...
  renderToggleableIcons: function (icon) {
    this.$("#toggleableIcons ul").append(
      new ToggleableIconView({
        tagName: "li",
        model: icon,
      }).el,
    );
  ...
  renderBackgroundOptions: function (backgroundOption) {
    this.$("#backgroundOptions ul").append(
      new BackgroundOptionView({
        tagName: "li",
