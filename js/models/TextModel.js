var Text = Backbone.Model.extend({
  ...
  draw: function (context, y) {
    if (this.get("enabled")) {
      ...
      if (this.get("multiline")) {
        ...
        context.fillText(this.get("content"), this.get("x"), y);
        context.strokeText(this.get("content"), this.get("x"), y);
      }
    }
  ...
  _wrapText: function (context, text, x, y, maxWidth, lineHeight) {
    ...
    var fragments = _.map(
      text.split(" "),
      function (word, key, list) {
        var words;
        if (/^\*\*(.+)\*\*/.test(word)) {
          // Word contains ** **
          words = _.map(
            word.split("**").slice(1, 3),
            function (word, key) {
              if (key == 0) {
    ...
    while (fragments.length > 0) {
      ...
      while (
        _.reduce(
          fragments.slice(0, lineWordCount + 1),
          function (total, word) {
            return total + word["length"];
          },
          0,
        ) +
          (lineWordCount - 1) * spaceWidth <
          maxWidth &&
        lineWordCount <= fragments.length
      ) {
        lineWordCount += 1;
      }
      var lineLength =
        _.reduce(
          fragments.slice(0, lineWordCount),
          function (total, word) {
            return total + word["length"];
          },
          0,
        ...
        function (fragment) {
          _.each(
            ...
            function (word) {
              if (word["word"] == "$P$") {
                console.log(
                  JSON.stringify([lineX, y, word["size"], word["size"]]),
                );
...
var ClassRestrictionText = Text.extend({
  ...
  preDraw: function (context, y) {
    ...
    if (testWidth > 540) {
      ...
      if (testWidth > 540) {
        context.drawImage(
          images["img/classRestrictionHighlight-Multiline.png"],
          450 - 302,
          y - 82,
        );
        ...
        context.drawImage(
          images["img/classRestrictionHighlight.png"],
          450 - 302,
          y - 68,
        );
...
var Dice2D6 = Dice.extend({
  ...
  draw: function (context, y) {
    ...
    if (this.get("enabled")) {
      ...
      if (this.get("doubleShadow")) {
        context.shadowOffsetX = this.get("shadowOffsetX") - 900;
        context.fillText(this.get("content"), this.get("x") + 120 + 900, y);
        context.shadowOffsetX = this.get("shadowOffsetX");
      }

      context.fillText(this.get("content"), this.get("x") + 120, y);
    }
  ...
  draw: function (context, y) {
    ...
    if (this.get("enabled")) {
      ...
      if (this.get("doubleShadow")) {
        ...
        context.fillText(diceResults[0], this.get("x") - 70 + 900, y);
        context.fillText(diceResults[1], this.get("x") + 70 + 900, y);

        context.shadowOffsetX = this.get("shadowOffsetX");
      }

      context.fillText(diceResults[0], this.get("x") - 70, y);
      context.fillText(diceResults[1], this.get("x") + 70, y);
    }
