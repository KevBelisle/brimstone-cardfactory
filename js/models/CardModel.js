...
var Card = Backbone.Model.extend({
	...
	drawCard: function () {
		...
		if (window.location.protocol != "file:") {
			...
			//take apart data URL
			var parts = this.get("context").canvas.toDataURL().match(/data:([^;]*)(;base64)?,([0-9A-Za-z+/]+)/);
			...
			ga('send', 'event', { eventCategory: 'General Use', eventAction: 'Card Draw', eventLabel: 'Clicked'});
		}
	},
	
	drawBackground: function () {
		if (this.get("backgroundOptions").length > 0) {
			this.get("context").drawImage(images[this.get("backgroundOptions").findWhere({enabled: true}).get("img")], 0, 0);
		} else {
	...
	drawToggleableText: function () {
		_.each(this.get("toggleableText"),
			function(text) {
				text.draw(this);
			},
			this.get("context")
...
var ItemCard = Card.extend({
	...
	drawToggleableText: function () {
		...
		if ( _.any(tI, function(i){ return i.get("y") > 400 && i.get("enabled") } )  ) {
			baseline = 940;
		} else {
			baseline = 1030;
		}
	...
	/*	_.each(
		...
		_.each(
			...
			function(tIKey) {
				...
				if (this.get("toggleableText")["coin"].get("enabled")
					&& ["singleHand", "twoHand", "threeHand"].indexOf(tIKey) > -1
				) {
					tI.draw(this.get("context"), tI.get("x")-100);
				
				} else if (this.get("toggleableText")["coin"].get("enabled")
	...
	constructor: function() {
		...
		this.set("requiredText", {
			...
			description: new CardText({
				label: "Description",
				content: "for dragging unsuspecting townsfolks screaming into the darkness.",
...
var ConditionCardBack = CardBack.extend({
	constructor: function() {
		...
		this.set("backgroundOptions", new Backbone.Collection([
			new BackgroundOption("Mutation Card Back", "img/cardback-mutation.jpg", true),
